...
char* longAddition( char* summand1,  char* summand2)
{
	unsigned long long summand1Size, summand2Size, resultSize, shortest, accomulator = 0, newSize;
	char *result; 
	unsigned int sum = 0;
	bool summand1IsShorter;	

	summand1Size = strlen(summand1);
	summand2Size = strlen(summand2);
	resultSize = (summand1Size >= summand2Size) ? summand1Size+1 : summand2Size+1;
	...
	shortest = (summand1Size <= summand2Size) ? summand1Size : summand2Size;
	summand1IsShorter = (summand1Size <= summand2Size) ? true : false;
	do
	{
		resultSize--;
		if(shortest > 0)
